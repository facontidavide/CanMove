...
//
// $Id: libpcan.c 753 2014-01-21 10:45:03Z stephane $
//
//****************************************************************************
...
DWORD CAN_Init(HANDLE hHandle, WORD wBTR0BTR1, int nCANMsgType)
{
  ...
  if (desc)
  {
    ...
    if ((err = __ioctl(desc->nFileNo, PCAN_INIT, &init)) < 0)
      return err;
  }
...
DWORD CAN_Status(HANDLE hHandle)
{
  ...
  if (desc)
  {
    ...
    if ((err = __ioctl(desc->nFileNo, PCAN_GET_STATUS, &status)) < 0)
    {
      return err; 
    }
    else
...
DWORD CAN_Write(HANDLE hHandle, TPCANMsg* pMsgBuff)
{
  ...
  if (desc)
  {
    if ((err = __ioctl(desc->nFileNo, PCAN_WRITE_MSG, pMsgBuff)) < 0)
      return err;
  }
...
DWORD CAN_Read(HANDLE hHandle, TPCANMsg* pMsgBuff)
{
  ...
  if (desc)
  {
    ...
    if ((err = __ioctl(desc->nFileNo, PCAN_READ_MSG, &m)) < 0)
      return err;
...
DWORD LINUX_CAN_Extended_Status(HANDLE hHandle, int *nPendingReads, int *nPendingWrites)
{
  ...
  if (desc)
  {
    ...
    if ((err = __ioctl(desc->nFileNo, PCAN_GET_EXT_STATUS, &status)) < 0)
      return err; 
    else
		{
